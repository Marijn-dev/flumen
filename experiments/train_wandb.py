...
def main():
    ...
    model_name = f"flumen-{timestamp}-{data_path.stem}-{run.id}"

    # Prepare for saving the model
    model_save_dir = Path(
        f"{sys_args.outdir}/{first_name}/{full_name}_{run.id}"
    )
    ...
    seed = wandb.config.get("model_key_seed", 0)
    ...
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(
        ...
        cooldown=0,
        factor=1.0 / wandb.config["sched_factor"],
    )

    loss = get_loss(wandb.config["loss"]).to(device)

    early_stop = EarlyStopping(
        es_patience=wandb.config["es_patience"],
        es_delta=wandb.config["es_delta"],
    )

    bs = wandb.config["batch_size"]
    seed = wandb.config.get("torch_seed", 0)
    g = torch.Generator()
    g.manual_seed(seed)
